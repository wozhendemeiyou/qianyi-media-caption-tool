...
本手册对应 v3.6.demo。公开版不带 API Key 和提示词模板，所有凭据与模板均由用户在自己的 Windows 账户中配置。
...
1. **左侧导航**：项目中心、图像打标、视频反推、单次反推、平台设置、系统说明；
...
外部 API 与本地模型互斥：外部 API 模式会锁定本地模型配置，本地模型模式会将供应商、模型、Base URL 和 API Key 区域整体置灰。API Key 不写入仓库或普通 JSON，而是使用 Windows DPAPI 分平台加密。内置服务商不显示 Base URL，只需要填写 API Key；选择自定义兼容接口后才会展开 Base URL 与 API Key。

本地运行方式有三种：

- **Hugging Face 本地目录**：选择包含 `config.json` 的视觉语言模型目录；单个 `.safetensors` 或 `.bin` 权重也可以，不要求分片。
- **LM Studio 本地服务**：连接 LM Studio 的 OpenAI 兼容服务，并在下拉框中加载/卸载视觉模型。
- **llama.cpp 原生 GGUF**：选择 `llama-server.exe`、主模型 `.gguf` 和匹配的 `mmproj*.gguf`。开始任务时程序自动启动 llama-server，任务结束、取消或失败时自动关闭，不经过 LM Studio。

llama.cpp 模式的“上下文”默认是 4096，“GPU 层”默认是 99。显存不足时先降低 GPU 层数，再降低上下文；并发建议从 1 开始。服务器启动和错误日志会写入本机应用数据目录的 `logs/llama-cpp/`，便于排查模型架构、显存或 `mmproj` 不匹配问题。
...
不需要建立项目时，可从项目中心顶部或左侧“单次反推”直接处理一个素材。这个工作区沿用平台设置、提示词、输出语言和思考标签清理选项，但不会切换当前项目目录或批量工作流模式。

- **图片**：支持点击选择、拖放以及 `Ctrl+V` 粘贴剪贴板图片。完成后可直接编辑、复制或另存 TXT；未点击保存前不会改写原图片旁的 sidecar TXT。
- **音视频**：支持 MP4、MOV、AVI、MP3、WAV、M4A、AAC、FLAC 和 OGG。文件载入后会读取时长与音视频轨道，并显示内嵌预览和时间轴。
- **手工选区**：拖动开始与结束滑块，最短片段为 0.25 秒；可以恢复全长、播放选区、仅截取保存，或“截取并反推”。
- **音轨控制**：视频存在音频轨时可选择是否保留；纯音频会自动封装为带静态画面的 MP4，以便交给兼容视频输入的平台。
- **独立状态**：单次反推显示自己的进度、耗时、字数和速度，不会清空项目缩略图、扫描状态或当前选择。
...
外部 API 与本地模型均支持 1–10 并发。选择本地模型时，界面会提示并发数越高显存与内存占用越大；建议从 1 开始，再根据硬件余量和模型稳定性逐步提高。生成请求不会自动重试，以降低重复计费风险。
