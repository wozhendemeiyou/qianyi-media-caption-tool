...
本手册对应 v3.5。公开版不带 API Key 和提示词模板，所有凭据与模板均由用户在自己的 Windows 账户中配置。
...
API Key 不写入仓库或普通 JSON，而是使用 Windows DPAPI 分平台加密。公开服务商地址由程序内置，用户不需要复制 Base URL。
...
外部 API 支持 1–10 并发。本地模型固定单并发，避免显存竞争。生成请求不会自动重试，以降低重复计费风险。
...
系统说明页显示当前版本、功能说明、更新记录、备份与诊断入口。发现新版后，工作区会显示更新提示；用户确认后可直接在软件内下载、校验、覆盖并重启，不需要手动前往下载页。
